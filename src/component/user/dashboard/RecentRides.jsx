                            ...
                            {recentRides.length > 0 ? recentRides.map((ride, idx) => (
                                <tr key={idx} className="hover:bg-slate-50/50 transition-colors">
                                    ...
                                    <td className="px-5 py-3.5">
                                        ...
                                        <p className="text-sm text-slate-600 font-medium">
                                            {ride.bikeId?.registration_number || ride.bikeId?.model_name || "—"}
                                        </p>
                                        <p className="text-[10px] text-slate-400">
                                            {ride.startStationId?.name || "—"} → {ride.endStationId?.name || "Ongoing"}
                                        </p>
                                    </td>
                                    <td className="px-5 py-3.5">
                                        <span className={`text-[10px] font-bold px-2 py-0.5 rounded-md uppercase ${statusStyles[ride.status] || "bg-slate-100 text-slate-500"
                                            }`}>
                                            {ride.status}
                                        </span>
                                    </td>
                                ...
                                <tr>
                                    <td colSpan={4} className="px-5 py-10 text-center text-xs font-bold text-slate-400">
                                        No recent rides found. Time to explore!
                                    </td>
                                </tr>
                            )}
                        </tbody>
